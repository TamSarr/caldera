 ...
 */
add_filter( 'template_include', array( 'Caldera_Wrapping', 'wrap' ), 99 );
add_filter( 'caldera_wrap_base', 'caldera_wrap_base_cpts' );
 ...
 */
function caldera_template_path() {
	return Caldera_Wrapping::$main_template;
}
 ...
 */
function caldera_template_base() {
	return Caldera_Wrapping::$base;
}

if ( ! class_exists( 'Caldera_Wrapping' ) ) {
	...
	class Caldera_Wrapping {
		...
		/**
		 * Initialize new instance of Caldera_Wrapping class by setting a variable for the $slug (which defaults to base)
		...
		public function __toString() {
			$this->templates = apply_filters( 'caldera_wrap_' . $this->slug, $this->templates );
		 ...
		 * and $base as static variables in Caldera_Wrapping class.
		 *
		 * @since 1.0.0
 ...
 */
function caldera_wrap_base_cpts( $templates ) {
