...
if ( ! class_exists( 'Caldera_Widget_Area' ) ) {

	class Caldera_Widget_Area {
		...
		function __construct() {
			...
			add_action( 'caldera_render_widget_area', array( $this, 'render' ) );
		}
		...
		public function render( $area_id ) {
			...
			$area_id        = apply_filters( 'caldera_rendering_current_widget_area', $area_id );
			...
			$classes = apply_filters( 'caldera_widget_area_classes', $classes, $area_id );
	...
	function caldera_widget_area() {
		return Caldera_Widget_Area::get_instance();
	}
